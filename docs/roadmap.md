...
| **Prove the methodology beats an unguided agent** | **Done once, with a mixed result.** The first treatment-vs-control comparison — same model, same commits, methodology vs. a plain request for a performance review — is in [`benchmark/ab-comparison.md`](../benchmark/ab-comparison.md) (protocol, committed before any control output was read) and [`benchmark/ab-results/first-comparison.md`](../benchmark/ab-results/first-comparison.md) (results, both arms' raw output committed). **Supported:** large, consistent gains in epistemic discipline — three fabricated improvement percentages in the control against none in the treatment, near-total absence of falsifiers in the control, and no evidence-grading axis in control output at all. **Not supported:** breadth. The control reported ~2.5× more findings, avoided both `forbidden` traps unaided, and surfaced two real issues the corpus did not contain (now `GT-A09`/`GT-A10`, marked as control-arm in origin). n = 3, one model — directional, not general. The breadth gap is the open defect it exposed |
