...
| A live GitHub Actions dry run of `action.yml` | **Done, for the safe half.** A permanent CI job (`.github/workflows/checks.yml`) now runs `action.yml` itself via `uses: ./` against the committed example on every push and PR — not a one-off manual check — and asserts the produced SARIF and outputs are correct. Verified on a real run (`gh run view` against the actual GitHub Actions log, not just a green checkmark). Deliberately runs with `comment: false` / `upload-sarif: false`, since posting a real PR comment or code-scanning upload on every push would be noise. **Still open:** a real, one-time run with both enabled, against an actual open pull request — genuinely outward-facing (visible comment, real code-scanning upload), so it needs an explicit decision to run rather than happening as a side effect of CI |
